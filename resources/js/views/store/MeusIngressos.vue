    ...
    <div>
        ...
        <div class="flex align-items-center gap-2 mb-4">
            ...
            <div>
                ...
                <small class="text-color-secondary">{{ ingressos.length }} ingresso(s) encontrado(s)</small>
            </div>
        </div>
        ...
        <!-- Vazio -->
        <div v-else-if="ingressos.length === 0" class="card text-center py-6">
            ...
            <p class="text-color-secondary mb-4">Você ainda não comprou ingressos nesta atlética.</p>
        ...
        <div v-else class="grid">
            ...
            >
                <div class="ingresso-ticket" :class="`status-${ingresso.status}`">
                    ...
                    <!-- Header do ingresso (banner do evento) -->
                    <div class="ingresso-ticket-header">
                        <img
                            v-if="ingresso.evento.banner_url"
                            :src="ingresso.evento.banner_url"
                            class="ingresso-banner"
                        />
                        <div class="ingresso-header-overlay">
                            <Tag
                                :value="statusLabel(ingresso.status)"
                                :severity="statusSeverity(ingresso.status)"
                                class="mb-2"
                            />
                    ...
                    <div class="ingresso-ticket-body">
                        ...
                        <!-- QR code: exibe para paid E reserved -->
                        <div v-if="ingresso.status === 'paid' || ingresso.status === 'reserved'" class="qr-wrapper">
                            <img
                                v-if="qrUrls[ingresso.id]"
                                :src="qrUrls[ingresso.id]"
                                class="qr-img"
                                alt="QR Code"
                            />
...
const qrUrls    = ref({});   // id → data URL do QR Code
...
async function gerarQRCodes() {
    // toDataURL não depende de elemento DOM — zero problemas de timing
    ...
    for (const ingresso of ingressos.value) {
        if (ingresso.status === 'paid' || ingresso.status === 'reserved') {
            try {
                ...
                console.error('QR Code error:', ingresso.code, e);
            }
        }
    }
}
...
/* ── Placeholder (pendente/usado/cancelado) ──────────── */
.qr-placeholder {
    display: flex;
    flex-direction: column;
    align-items: center;
...
.qr-placeholder.pending   { background: #fffbeb; }
